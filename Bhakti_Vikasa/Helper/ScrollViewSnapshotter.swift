...
//  Bhakti_Vikasa
//
//  Created by gagandeepmishra on 27/01/21.
...
class ScrollViewSnapshotter: NSObject {
    static let shared = ScrollViewSnapshotter()
        ...
        if let context = UIGraphicsGetCurrentContext()
        {
            for indexHorizontal in 0 ..< numberOfPagesThatFitHorizontally
            {
                for indexVertical in 0 ..< numberOfPagesThatFitVertically
                {
                    ...
                    scrollview.contentOffset = CGPoint(x: offsetHorizontal, y: offsetVertical)//CGPointMake(offsetHorizontal, offsetVertical)
